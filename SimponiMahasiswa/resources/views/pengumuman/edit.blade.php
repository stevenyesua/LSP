...
<div class="bg-white mx-96 p-8 text-xl  gap-8">
    <form method="POST" action="{{route('pengumuman.update', ['id' => $pengumuman->id])}}"enctype="multipart/form-data">
    @csrf
    @method('PATCH')
        <div class="relative mt-4">
            <input value="{{ $pengumuman->judul }}" maxlength="100" name="judul" type="text" id="judul" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
        ...
        <div class="relative mt-4">
            <textarea name="deskripsi" id="deskripsi" rows="4" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" ">{{ $pengumuman->deskripsi }}</textarea>
