...
<div class="bg-white mx-96 p-8 text-xl flex gap-8">
    <div class="kiri-detail-user">
        <img src="{{ asset('storage/' . $collection->user->foto) }}" alt="Image" class="rounded-lg w-72" style="box-shadow: 0px 3px 1px 0px rgba(0, 0, 0, 0.25);">
    ...
    <div class="kanan-detail-user">
        ...
        <div class="informasi-user flex">
            ...
            <div class="mt-4 ml-16 text-sm font-medium text-gray-600">
                ...
                : {{($collection->alamat)}} <br>
                : {{($collection->user->asal_sekolah)}} <br>
            </div>
        </div> 
        <div class="mt-4 text-sm font-medium text-gray-600">
        ...
        <div class="informasi-user flex">
            <div class="mt-4 mr-2 text-sm font-medium text-gray-600">
                Jurusan <br>
                Waktu <br>
            </div>
            <div class="mt-4 ml-24 text-sm font-medium text-gray-600">
                : {{($collection->jurusan)}} <br>
                : {{($collection->waktu_kuliah)}} <br>
            </div>
        </div> 
        <div class="status-akun flex text-sm font-medium text-gray-600">
            ...
            <div class="ml-8">
                ...
                <div class="status-df flex gap-2">
                    <div>:</div>
                    <form action="{{route('mahasiswa.validate_admin',['id'=>$collection->id])}}" method="POST">
                    @method('PATCH')
                    @csrf
...
<!-- Button to open modal -->
            ...
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                ...
                <div class="p-4 md:p-5 space-y-4 flex bukti-foto">
                    <img src="{{ asset('storage/' . $collection->hasil_test) }}" alt="Image" class="rounded-lg w-full" style="box-shadow: 0px 3px 1px 0px rgba(0, 0, 0, 0.25);">
                </div>
                </div>
            </div>
            </div>
